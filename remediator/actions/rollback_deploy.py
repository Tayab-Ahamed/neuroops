...
def rollback_deployment(namespace: str, deployment_name: str) -> ActionResult:
    ...
    """
    logger.info("Starting rollback_deployment action", namespace=namespace, deployment_name=deployment_name)
    start_time = time.time()

    if not k8s_configured:
        logger.info("remediator mock: rollback_deployment execution", namespace=namespace, deployment_name=deployment_name)
        time.sleep(0.5)
    ...
    try:
        ...
        if not current_revision:
            logger.warning("No current revision found in deployment annotations, assuming 1", deployment=deployment_name)
            current_revision = "1"
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                if (observed_generation >= generation and 
        ...
        duration = time.time() - start_time
        if rollout_complete:
            logger.info("Rollback rollout completed successfully", deployment=deployment_name, duration=duration)
            return ActionResult(
                success=True,
                action_taken=(
            ...
            logger.error("Rollback rollout verification timed out", deployment=deployment_name, duration=duration)
            return ActionResult(
                success=False,
                action_taken=(
        ...
        duration = time.time() - start_time
        logger.error("Failed to execute rollback_deployment", deployment=deployment_name, error=str(e))
        return ActionResult(
            success=False,
