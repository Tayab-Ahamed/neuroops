...
def prompt_human(hypothesis: Union[Dict[str, Any], Any], action: str) -> bool:
    """
    ...
    if "rollback" in action_lower:
        risk_level = "HIGH"
        risk_color = "red"
        risk_desc = "Reverts deployment image & configurations to a prior revision. High potential impact."
    elif action_lower in ("restart", "scale", "patch_configmap", "scale_replicas"):
        risk_level = "MEDIUM"
        risk_color = "yellow"
        risk_desc = "Modifies active resource replica scales or restarts active container instances."
    ...
    console.print()
    title_text = Text(f"🚨 NEUROOPS INCIDENT RESOLUTION PANEL [{incident_id}] 🚨", style="bold white on red", justify="center")
    ...
    console.print(Panel(table, title="[bold white]Incident Overview[/bold white]", border_style=risk_color))
    ...
    if approved:
        console.print("[bold green]✔ Remediation action approved. Launching recovery...[/bold green]\n")
