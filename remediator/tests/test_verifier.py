...
def test_verify_resolution_success():
    ...
    with patch("httpx.Client", return_value=mock_client), \
        ...
        assert verify_resolution({"id": "alert-123", "service": "backend"}, timeout_seconds=10) is True
...
def test_verify_resolution_timeout():
    ...
    with patch("httpx.Client", return_value=mock_client), \
         ...
         patch.dict(os.environ):
        os.environ.pop("REMEDIATOR_TEST_APPROVAL", None)
        assert verify_resolution({"id": "alert-123", "service": "backend"}, timeout_seconds=10) is False
