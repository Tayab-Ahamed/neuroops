...
from remediator.human_loop import input_with_timeout, prompt_human
...
def test_prompt_human_risk_categories():
    ...
    with (
        patch("remediator.human_loop.input_with_timeout", return_value="y"),
...
def test_prompt_human_rejection():
    ...
    with (
        patch("remediator.human_loop.input_with_timeout", return_value="n"),
        patch.dict(os.environ),
    ):
        os.environ.pop("REMEDIATOR_TEST_APPROVAL", None)
        res = prompt_human(
