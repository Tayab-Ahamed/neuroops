...
try:
    ...
    logger.warning(
        "Failed to configure OTel OTLP exporter, falling back to Console Span Processor",
        error=str(e),
    )
            ...
            with tracer.start_as_current_span(f"agent.{agent_name}") as span:
                ...
                if isinstance(result, dict):
                    ...
                    if isinstance(findings, dict):
                        decision = (
                            findings.get("hypothesis")
                            or findings.get("recommended_action")
                            or findings.get("likely_origin")
                ...
                logger.info(
                    "Agent node execution completed", agent=agent_name, latency_ms=latency_ms
                )
                return result
