...
def query_metric(promql: str, minutes: int = 10) -> str:
    ...
    if not prom_configured:
        ...
        return f"PromQL Query: {promql}\nTime Series Results:\n" + "\n".join(
            [f"{ts:.1f}: {val}" for ts, val in points]
        )
...
def compare_services(metric: str, time_window: str = "5m") -> str:
    ...
    try:
        ...
        if "latency" in metric.lower():
            ...
            queries = {
                "frontend": 'sum(rate(container_cpu_usage_seconds_total{container="frontend-service"}[1m]))',
                "backend": 'sum(rate(container_cpu_usage_seconds_total{container="backend-service"}[1m]))',
                "database": 'sum(rate(container_cpu_usage_seconds_total{container="database-service"}[1m]))',
            }
