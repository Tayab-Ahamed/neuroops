...
from agents.detective import detective_node, DetectiveOutput
from agents.topologist import topologist_node, TopologistOutput
from agents.historian import historian_node, HistorianOutput
from agents.supervisor import supervisor_synthesize_node, supervisor_init_node, SupervisorOutput
...
async def test_historian_node_llm_path(mock_chat_anthropic, monkeypatch):
    ...
    mock_tc_res.tool_calls = [{
        ...
        "args": {"repo": "neuroops/app"},
        "id": "call_789"
    }]
    
    final_output = HistorianOutput(
        recent_deploys=[{"commit": "a1b2"}],
        suspect_commit="a1b2",
        deploy_time="2026-05-22"
    )
