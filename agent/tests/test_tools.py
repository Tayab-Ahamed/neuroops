...
from tools import github_tools, jaeger_tools, k8s_tools, prometheus_tools
...
def test_k8s_get_pod_status_mock():
    ...
    # 1. Test OOM pod name
    res_oom = k8s_tools.get_pod_status.func(
        **{"namespace": "default", "pod_name": "backend-oom-123"}
    )
    assert "OOMKilled" in res_oom

    # 2. Test Crash pod name
    res_crash = k8s_tools.get_pod_status.func(
        **{"namespace": "default", "pod_name": "backend-crash-123"}
    )
    assert "CrashLoopBackOff" in res_crash

    # 3. Test default pod name
    res_normal = k8s_tools.get_pod_status.func(
        **{"namespace": "default", "pod_name": "backend-normal"}
    )
...
def test_k8s_get_pod_status_live(mock_core_api):
    ...
    res = k8s_tools.get_pod_status.func(**{"namespace": "default", "pod_name": "backend-pod"})
...
def test_k8s_get_deployment_history_mock():
    k8s_tools.k8s_configured = False
    res = k8s_tools.get_deployment_history.func(
        **{"namespace": "default", "deployment_name": "backend"}
    )
...
def test_k8s_get_recent_events_mock():
    k8s_tools.k8s_configured = False
    res = k8s_tools.get_recent_events.func(
        **{"namespace": "default", "service_name": "backend", "minutes": 10}
    )
...
def test_github_get_recent_deploys_mock():
    ...
    res = github_tools.get_recent_deploys.func(**{"repo": "neuroops/org", "minutes": 60})
...
def test_prometheus_query_metric_mock():
    ...
    res = prometheus_tools.query_metric.func(**{"promql": "up{job='backend'}", "minutes": 10})
...
def test_prometheus_query_metric_live(mock_prom_class):
    ...
    mock_res = [{"metric": {"job": "backend-service"}, "values": [[1716390000, "1.0"]]}]
    ...
    with patch("tools.prometheus_tools.prom", mock_prom_instance):
        res = prometheus_tools.query_metric.func(**{"promql": "up", "minutes": 10})
...
def test_prometheus_compare_services_mock():
    ...
    res_error = prometheus_tools.compare_services.func(**{"metric": "error", "time_window": "5m"})
    assert "Error Rate" in res_error
    assert "backend: 0.22" in res_error

    res_latency = prometheus_tools.compare_services.func(
        **{"metric": "latency", "time_window": "5m"}
    )
    assert "P95 Latency" in res_latency
    assert "frontend: 2.15s" in res_latency

    res_cpu = prometheus_tools.compare_services.func(**{"metric": "cpu", "time_window": "5m"})
    assert "Saturation" in res_cpu
    assert "backend: CPU: 95%" in res_cpu

    res_traffic = prometheus_tools.compare_services.func(
        **{"metric": "traffic", "time_window": "5m"}
    )
...
def test_prometheus_compare_services_live(mock_prom_class):
    ...
    with patch("tools.prometheus_tools.prom", mock_prom_instance):
        ...
        res_error = prometheus_tools.compare_services.func(
            **{"metric": "error", "time_window": "5m"}
        )
...
def test_prometheus_compare_services_live_error(mock_prom_class):
    ...
    with patch("tools.prometheus_tools.prom", mock_prom_instance):
        res = prometheus_tools.compare_services.func(**{"metric": "latency", "time_window": "5m"})
...
def test_jaeger_get_service_dependencies_mock():
    ...
    res_front = jaeger_tools.get_service_dependencies.func(**{"service_name": "frontend"})
    assert (
        "upstream_services" not in res_front
    )  # wait, mock format matches jaeger_tools.py: Service Dependency Analysis for frontend
    assert "Service Dependency Analysis for frontend" in res_front
    assert "Bottleneck: backend-service" in res_front

    res_back = jaeger_tools.get_service_dependencies.func(**{"service_name": "backend"})
    assert "Upstream: frontend-service" in res_back
    assert "Bottleneck: backend-service self-latency" in res_back

    res_db = jaeger_tools.get_service_dependencies.func(**{"service_name": "database"})
...
def test_jaeger_get_service_dependencies_live(mock_http_get):
    ...
    res = jaeger_tools.get_service_dependencies.func(**{"service_name": "backend"})
