...
def get_llm(structured_schema: Optional[Type[T]] = None) -> Any:
    ...
    if not provider:
        if os.getenv("ANTHROPIC_API_KEY"):
            ...
            provider = "ollama" if os.getenv("OLLAMA_MODEL") or os.getenv("OLLAMA_BASE_URL") else "mock"
    ...
    if provider == "anthropic":
        ...
        logger.info("Initializing local Ollama model via ChatOpenAI compatibility", model=model_name, base_url=base_url)
