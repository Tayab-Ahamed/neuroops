...
class LogAnalyserOutput(BaseModel):
    error_logs: list[str] = Field(
        description="List of raw error log entries parsed from container output"
    )
    suspect_stack_trace: str | None = Field(
        description="Extracted traceback, panic, or core exception message"
    )
    reasoning: str = Field(
        description="Analytical explanation of log errors and exception patterns"
    )
...
async def log_analyser_node(state: AgentState) -> dict:
    ...
    if llm is None:
        ...
        if service == "backend":
            ...
            return {
                "log_findings": {
                    "error_logs": ["FATAL: write-ahead log write failed: disk full"],
