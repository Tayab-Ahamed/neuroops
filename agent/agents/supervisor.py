...
class SupervisorOutput(BaseModel):
    hypothesis: str = Field(description="The synthesized root cause hypothesis detailing the diagnosed failure mode")
    confidence: float = Field(description="Confidence rating from 0.0 to 1.0 indicating diagnostic certainty")
    recommended_action: str = Field(description="The mapped remediation action from decision tree (rollback, restart, scale, none)")
    requires_human_approval: bool = Field(description="Flag indicating if the action requires human operator review")
