...
class LSTMAnomalyModel:
    ...
    def __init__(self, sequence_length: int = 5, alpha: float = 1.0, **kwargs):
        ...
        logger.info("Initialized sequence predictor model", sequence_length=sequence_length, features=self.features)
    ...
    def _prepare_sequences(self, windows: List[MetricWindow]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(len(windows) - self.sequence_length):
            ...
            seq = np.concatenate([self._extract_features(windows[i + j]) for j in range(self.sequence_length)])
    ...
    def fit(self, windows: List[MetricWindow]):
        ...
        for service, wins in service_windows.items():
            if len(wins) < self.sequence_length + 5:
                logger.warn("Too few baseline samples to train sequence predictor", service=service, sample_count=len(wins))
                continue
            ...
            logger.info("Trained sequence model successfully", service=service, threshold=self.thresholds[service])
    ...
    def score(self, sequence: List[MetricWindow], next_window: MetricWindow) -> float:
        ...
        seq_features = np.concatenate([self._extract_features(w) for w in sequence[-self.sequence_length:]])
