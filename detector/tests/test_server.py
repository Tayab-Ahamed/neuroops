...
with patch("scraper.PrometheusConnect") as mock_prom:
    from server import app, active_alerts, model, scraper
    import server
...
def test_startup_model_load_exception():
    ...
    with patch("os.path.exists", return_value=True), \
         patch("models.isolation_forest.IsolationForestModel.load", side_effect=Exception("Failed to load")), \
...
def test_lifespan_startup_shutdown():
    ...
    with patch("server.background_scraping_loop", side_effect=mock_loop), \
         patch("scraper.PrometheusConnect"):
