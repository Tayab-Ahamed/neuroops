...
def test_execute_run_live_success():
    ...
    mock_alert_resp.json.return_value = [{"service": "backend", "id": "alert-123", "severity": "P1"}]
...
def test_cli_mock_runs():
    ...
    with patch("benchmarks.runner.verify_k8s_reachable", return_value=False), \
...
def test_cli_live_runs_all_online():
    runner = CliRunner()
    with patch("benchmarks.runner.verify_k8s_reachable", return_value=True), \
...
def test_cli_report_compilation_exception():
    ...
    with patch("benchmarks.runner.verify_k8s_reachable", return_value=False), \
         patch("benchmarks.runner.verify_service_reachable", return_value=False), \
         patch("benchmarks.report.compile_report", side_effect=Exception("Disk full")) as mock_report, \
...
def test_execute_run_live_detection_exception_retry():
    ...
    mock_rca_resp = MagicMock(status_code=200)
    mock_rca_resp.json.return_value = {"hypothesis": "Crash", "confidence": 0.85, "requires_human_approval": False}
    
    mock_rem_resp = MagicMock(status_code=200)
    mock_rem_resp.json.return_value = {"success": True, "action_taken": "restart", "duration_seconds": 1.0}
    
    mock_resolution_resp = MagicMock(status_code=200)
    mock_resolution_resp.json.return_value = []
    
    mock_client.get.side_effect = [Exception("Temporary glitch"), mock_alert_resp, mock_resolution_resp]
    mock_client.post.side_effect = [mock_rca_resp, mock_rem_resp]
...
def test_execute_run_live_cleanup_exception():
    ...
    with patch("subprocess.run") as mock_sub, \
        ...
        res = execute_run("pod-delete", "backend", 1, False, "http://det", "http://agent", "http://rem")
        assert res["status"] == "success"
        assert any("Cleanup during resolution polling failed" in call[0][0] for call in mock_logger.warning.call_args_list)
...
def test_execute_run_live_verifier_exception_retry():
    ...
    mock_client.get.side_effect = [mock_alert_resp, Exception("Temp verifier glitch"), mock_resolution_resp]
    mock_client.post.side_effect = [mock_rca_resp, mock_rem_resp]
...
def test_runner_main_execution():
    with patch("sys.argv", ["benchmarks/runner.py", "--scenario", "pod-delete", "--runs", "1", "--mock"]):
