...
def main(
    ...
    if not is_mock:
        ...
        if not (k8s_ok and detector_ok and agent_ok and remediator_ok):
            ...
            console.print(
                "[green]System status: All live APIs and Kubernetes cluster verified. Executing LIVE chaos.[/green]"
            )
...
def execute_run(
    ...
    if not is_mock:
        try:
            ...
            console.print(
                "[cyan][Chaos] LitmusChaos Engine applied successfully via kubectl.[/cyan]"
            )
        except Exception as e:
            ...
            return create_failed_run_payload(
                scenario, target_service, run_number, "Chaos Injection Failure"
            )
    else:
        console.print(
            f"[cyan][Chaos] (Mock) Injected fault: '{scenario}' on service '{target_service}'.[/cyan]"
        )

    # State variables for latencies
    t1: float | None = None
    t2: float | None = None
    t2_rca_end: float | None = None
    t3: float | None = None
    ...
    with Progress(
        ...
        detect_task = progress.add_task(
            "[yellow]Waiting for Anomaly Detection...[/yellow]", total=60
        )
        ...
        while time.time() - start_poll < timeout_duration:
            if is_mock:
                ...
                progress.update(
                    detect_task, completed=60, description="[green]✓ Alert Detected (Mock)[/green]"
                )
                break
            else:
                try:
                    with httpx.Client(timeout=3.0) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            target_alert = next(
                                (a for a in alerts if a.get("service") == target_service), None
                            )
                            if target_alert:
                                t1 = time.time()
                                ...
                                progress.update(
                                    detect_task,
                                    completed=60,
                                    description="[green]✓ Alert Detected![/green]",
                                )
                                break
                except Exception as e:
        ...
        if not t1:
            ...
            return create_failed_run_payload(
                scenario, target_service, run_number, "Detection Timeout"
            )

        # --- Task 2: Multi-Agent RCA (Call /investigate) ---
        rca_task = progress.add_task(
            "[yellow]Running Agent Diagnostics (RCA)...[/yellow]", total=100
        )
        t2_start = time.time()
        ...
        if is_mock:
            ...
            progress.update(
                rca_task, completed=100, description="[green]✓ RCA Finished (Mock)[/green]"
            )
        else:
            try:
                with httpx.Client(timeout=120.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        tokens_used = hypothesis_data.get(
                            "tokens_used", 5000
                        )  # Read from API or fall back
                        t2_rca_end = time.time()
                        progress.update(
                            rca_task, completed=100, description="[green]✓ RCA Finished[/green]"
                        )
                    else:
                        progress.update(rca_task, description="[red]✗ RCA Failed[/red]")
                        return create_failed_run_payload(
                            scenario, target_service, run_number, "Agent API Failed"
                        )
            except Exception as e:
                logger.error("RCA invocation failed", error=str(e))
                progress.update(rca_task, description="[red]✗ RCA Failed[/red]")
                return create_failed_run_payload(
                    scenario, target_service, run_number, "Agent Connection Error"
                )
        ...
        if is_mock:
            ...
            progress.update(
                remediator_task,
                completed=100,
                description="[green]✓ Remediation Applied (Mock)[/green]",
            )
        else:
            try:
                with httpx.Client(timeout=180.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        progress.update(
                            remediator_task,
                            completed=100,
                            description="[green]✓ Remediation Applied[/green]",
                        )
                    else:
                        progress.update(
                            remediator_task, description="[red]✗ Remediation Failed[/red]"
                        )
                        return create_failed_run_payload(
                            scenario, target_service, run_number, "Remediator API Failed"
                        )
            except Exception as e:
                ...
                return create_failed_run_payload(
                    scenario, target_service, run_number, "Remediator Connection Error"
                )
        ...
        while time.time() - start_verify < timeout_duration:
            if is_mock:
                ...
                progress.update(
                    verify_task,
                    completed=100,
                    description="[green]✓ Incident Fully Resolved (Mock)[/green]",
                )
                break
            else:
                try:
                        ...
                        if response.status_code == 200:
                            ...
                            if not target_alert:
                                ...
                                progress.update(
                                    verify_task,
                                    completed=100,
                                    description="[green]✓ Incident Fully Resolved[/green]",
                                )
                                break
                except Exception as e:
        ...
        if not t3:
            ...
            return create_failed_run_payload(
                scenario, target_service, run_number, "Resolution Verification Timeout"
            )
...
def create_failed_run_payload(
    scenario: str, target_service: str, run_number: int, error_message: str
) -> dict[str, Any]:
...
def render_scenario_summary(scenario: str, runs: list[dict[str, Any]]) -> None:
    ...
    for r in runs:
        status_str = (
            "[green]Success[/green]"
            if r["status"] == "success"
            else f"[red]Fail: {r['error_message']}[/red]"
        )
